...
class NewsAdapter(
    private var newsList: List<NewsResults>,
    private var mainActivityView: MainActivityInterface
    ...
    override fun onBindViewHolder(vh: MViewHolder, position: Int) {
        ...
        if (news.media.size > 0) {
            ...
            vh.main_relative.setOnClickListener(View.OnClickListener {
                mainActivityView.navigateToNewsDetails(
                    news.title,
                    news.abstract,
                    news.byline,
                    news.published_date,
                    news.media.get(0).media_metadata.get(1).url
                )
            })
        }
